...
export function evaluate(query: Query, min: number, max: number): NumberSet {
  ...
  if (query === "even") {
    ...
    const set = makeSortedNumberSet(evaluate(query.arg, min, max).getNumbers(min, max))
    set.complement();
    return set;
  } else if (query.kind === "and") {
    // TODO (5e): change to use .complement() from NumberSet instead
    // const set = complement(evaluate(query.left, min, max), min, max);
    const set = makeSortedNumberSet(evaluate(query.left, min, max).getNumbers(min, max));
    set.complement();
    // const notRight = complement(evaluate(query.right, min, max), min, max);
    const notRight = makeSortedNumberSet(evaluate(query.right, min, max).getNumbers(min, max));
    notRight.complement();
